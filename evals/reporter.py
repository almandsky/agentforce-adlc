...
def print_detailed(results: dict) -> None:
    ...
    for test in results["tests"]:
        ...
        # Pipeline steps
        pipeline = test.get("pipeline", [])
        pipeline_results = test.get("pipeline_results", {})
        if pipeline and len(pipeline) > 1:
            steps_str = []
            for step in pipeline:
                sr = pipeline_results.get(step, {})
                s_status = sr.get("status", "?")
                steps_str.append(f"{step}({s_status})")
            print(f"  Pipeline: {' -> '.join(steps_str)}")

        # Per-skill dimension scores
        skill_scores = test.get("skill_scores", {})
        if skill_scores:
            for skill, ss in sorted(skill_scores.items()):
                if ss:
                    print(f"  {skill}: {ss.get('overall', 0):.0f}%")

        if test.get("error"):
            print(f"  Error: {test['error']}")

        failed = [a for a in test.get("assertions_results", []) if a["result"] == "FAIL"]
        if failed:
...
def generate_markdown(results: dict) -> str:
    ...
    for test in results["tests"]:
        ...
        lines += ["| Assertion | Result | Reason |",
                  "|-----------|--------|--------|"]
        for a in test.get("assertions_results", []):
...
def main():
    ...
    p.add_argument("--format", "-f", default="summary",
                   choices=["summary", "detailed", "failures", "markdown", "json", "html"])
    p.add_argument("--output", "-o", help="Output file (default: stdout)")
    p.add_argument("--compare", help="Path to previous summary.json for HTML comparison")
    args = p.parse_args()
    ...
    if args.format == "summary":
        ...
        cmd = [sys.executable, str(report_script), args.results]
        if args.output:
